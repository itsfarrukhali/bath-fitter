...
import { Loader2, Pencil, HelpCircle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function EditSubcategoryModal({
  ...
  const [zIndex, setZIndex] = useState<number | "">(subcategory.z_index ?? 50);
  ...
  const fetchCategories = async () => {
    ...
    try {
      const { data } = await axios.get("/api/categories?forAdmin=true");
      if (data.success) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const { data } = await axios.put(
        `/api/subcategories/${subcategory.id}?forAdmin=true`,
        {
          name,
          slug,
